#!/usr/bin/env bash
# =============================================================================
# scripts/verify.sh — verify a downloaded Cackle release artifact, fail-closed.
#
...
# WHAT IT IS FOR
# ──────────────
# A Cackle release publishes one bare binary per platform plus a `checksums.txt`
...
#     name as a regex — every "." in a name like "cackle-darwin-arm64" is a wildcard
...
# USAGE
# ─────
#   scripts/verify.sh --tag v0.3.0 cackle-darwin-arm64
#   scripts/verify.sh --tag v0.3.0 --attest cackle-darwin-arm64
#   scripts/verify.sh --repo vul-os/cackle --tag v0.3.0 --out ~/Downloads ASSET
...
SELFTEST_ARTIFACT="cackle-linux-amd64"
...
trap cleanup EXIT
TMPDIR_V="$(mktemp -d "${TMPDIR:-/tmp}/cackle-verify.XXXXXX")"
...
run_selftest() {
  ...
  expect_exit "--attest but no gh installed"  "$E_ATTEST"         yes -- env PATH="$fakebin" "$bash_abs" "$SELF_PATH" --out "$outd" --attest --repo vul-os/cackle --base-url "$base/good" "$art"
