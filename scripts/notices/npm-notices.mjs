...
// package bundled into the shipped web UI.
//
...
//   1. from an override file in scripts/notices/npm-license-overrides/ (used
//      when upstream ships NO licence file — the override holds the real
//      upstream text with a note recording where it was verified), matched by
//      exact "<name>@<version>", then "<name>", then a "<name-prefix>" family
//      match;
...
// never be silently shipped. (scripts/gen-notices.sh treats that failure as
// "npm section omitted, don't block the rest of the build" — see there.)
...
if (existsSync(OVERRIDE_DIR)) {
  for (const f of readdirSync(OVERRIDE_DIR)) {
    ...
    const key = f.slice(0, -4).replace(/__/g, '/'); // e.g. "@scope/pkg"
...
function findOverride(name, version) {
  ...
  // like @foo/bar-darwin-arm64 matching a "@foo/bar" file).
  return (
