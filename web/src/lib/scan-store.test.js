...
import { recordScan, getPendingSync, markSynced, orderForUpload } from './scan-store.js';
 ...
 */
let scriptBatch = 0;
async function withScriptedIDs(count, fn) {
...
let eventCounter = 0;
function freshEventID(name) {
    eventCounter += 1;
    return `evt_${name}_${eventCounter}`;
}

/** Record `count` scans, one per ticket, in the given chronological order. */
async function recordSeries(eventID, { count, at, gateId = 'North', deviceId = 'device-A' }) {
    const recorded = [];
    for (let i = 0; i < count; i++) {
