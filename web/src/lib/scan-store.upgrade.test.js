...
async function seedVersion1(rows) {
...
function legacyRow({ id, ticket, at, result = 'admitted', note = null, synced = 0 }) {
    return {
        id,
        event_id: EVENT_ID,
        ticket_id: ticket,
...
const { recordScan, getPendingSync, getAdmissionsForEvent, orderForUpload } = await import('./scan-store.js');
...
test('the upgrade loses nothing that was queued', async () => {
    ...
    for (const original of LEGACY) {
        ...
        for (const field of ['event_id', 'ticket_id', 'device_id', 'gate_id', 'scanned_at', 'result', 'note', 'holder_name', 'synced']) {
            assert.deepEqual(found[field], original[field],
...
test('the upgrade backfills a sequence onto every record already queued', async () => {
    ...
    const seqOf = (ticket) => rows.find((r) => r.ticket_id === ticket).seq;
