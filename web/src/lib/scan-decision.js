 ...
 *
 * @param {object} args
 * @param {string} args.token the raw `cackle.<payload>.<sig>` string
 * @param {string} args.eventId the event this gate is admitting for
 * @param {Record<string, Uint8Array>} args.keyRing kid -> public key
 * @param {Set<string>} args.ticketIndexSet authoritative valid-ticket ids
 * @param {boolean} args.ticketIndexPresent whether that set is authoritative
 * @param {Set<string>} args.admittedIndexSet ids already admitted elsewhere
 *   as of when the bundle was built
 * @param {(token: string, ring: object, now: Date) => object} args.verify
 * @param {(eventId: string, ticketId: string) => Promise<boolean>} args.wasAdmitted
 * @param {Date} [args.now]
 * @returns {Promise<{result: string, note: string|null, ticketId: string|null, holderName: string|null}>}
 */
