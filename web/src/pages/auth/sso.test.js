...
import { signInProvidersFrom, ssoMessageFor, SSO_MESSAGES } from './sso.js';
...
test('the button row returns nothing when there are no providers', () => {
    const src = read('sso-buttons.jsx');
    assert.match(
        src,
        /if \(providers\.length === 0\) return null;/,
        'sso-buttons.jsx no longer short-circuits on an empty provider list — a button could render on a box with nothing configured',
    );
...
test('the app holds no absolute URL to any sign-in provider', () => {
    for (const name of ['sso.js', 'sso-buttons.jsx', 'signin.jsx', 'signup.jsx']) {
        const src = read(name);
