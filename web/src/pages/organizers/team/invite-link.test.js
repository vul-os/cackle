...
test('the invite link points at a route the app actually declares', () => {
    ...
    assert.match(
        ...
        `routes.jsx declares no ${INVITE_ACCEPT_PATH} route, so every invite link is dead`,
    );

    const acceptPage = read('pages/organizers/invite-accept.jsx');
    assert.match(
        acceptPage,
...
test('nothing in the invite flow claims an email was sent', () => {
    ...
    const files = {
        ...
        'pages/organizers/invite-accept.jsx': read('pages/organizers/invite-accept.jsx'),
    };
    const forbidden = [
        /Invite sent/i,
