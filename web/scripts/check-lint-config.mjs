 ...
 * cackle's copy — see the CONFIG block below for what differs.
 * ---------------------------------------------------------------------------
 */
...
const CONFIG = {
  ...
  // resolves. cackle's tsconfig.json has `"include": ["src", "vite.config.ts",
  // "tailwind.config.ts", "postcss.config.ts"]` — src is the app surface.
  fixtureDir: 'src',
  fixtureName: '__lint_config_gate_probe.ts',

  // Repo root to walk for package.json files (assertion B). cackle has
  // package.json at the repo root (playwright/tooling) and web/ (the app) —
  // a stray range on either is worth catching.
  ...
  // (assertion C). Measured real count at adoption time: 184 files (0 err,
  // 59 warn). Floor set to 150 — tolerates normal churn (adding/removing a
  // few dozen components) while still catching a glob that collapses to a
  // handful of files or zero.
  coverageFloor: 150,
}
