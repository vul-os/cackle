...
- **`FlowInvoice`** — the provider generates a payable invoice/request
  (crypto adapters, M-Pesa's STK push) rather than a page or a widget.
...
| M-Pesa (Safaricom Daraja) | `mpesa` | KE; KES only | Invoice (STK push — a PIN prompt on the buyer's phone, no redirect) | `CACKLE_MPESA_CONSUMER_KEY`, `CACKLE_MPESA_CONSUMER_SECRET`, `CACKLE_MPESA_SHORTCODE`, `CACKLE_MPESA_PASSKEY` | unit-tested — NOT sandbox-verified. Confidence HIGH on the overall shape (Safaricom's most stable, longest-documented integration). Daraja's STK callback carries **no signature at all** — `Webhook` never trusts it beyond "here's a request id to go check," and always re-verifies via the authenticated Query API. Rejects any amount with a nonzero cents remainder — M-Pesa's API only takes whole shillings. |
